...
dominating long-horizon returns ($\beta=0$). Second, portfolio heterogeneity is presented. Equal-weighted market portfolio resembles smaller firms, meanwhile, value-weighted market portfolio resembles larger firms. In the right panel, I have applied Newey-West standard error adjustment. Results 
are similar.\footnote{Notice that due to the limited amount of sample periods, Newey-West adjustment is actually not a good choice.}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item Return variance decomposition: They used the model that stock prices $(p_t)$ have two components: permanent $p_t^*$ $(p_t^*-p_{t-1}^*=\epsilon_t)$ and transitory AR(1) $u_t$ $(u_t=\rho_1u_{t-1}+\nu_t)$. Return $\Delta p_t$ is then $\Delta p_t = \epsilon_t + \frac{1-L}{1-\rho_1 L}\nu_t$. If $\epsilon_t$ and $\nu_t$ are independent, $\Delta p_t$ follows an ARMA(1,1) process. Then $\rho_1$ determines the persistence of the 
        transitory component, the relative magnitude of $\sigma^2_{\epsilon}$ and $\sigma^2_{\nu}$, defined as $\delta$, determines the importance of the two components in return movements. $\delta$ and $\rho_1$ will determine the return series.\footnote{This model was introduced by \citet{summers1986does}. $\Delta p_t = (p^*_t-p^*_{t-1})+(u_t-u_{t-1})$. After a backward induction, the AR(1) $u_t$ can be written as $u_t = \sum^{\infty}_{j=0}\rho_1^j \nu_{t-j}$. If we define $\nu_{t}=\nu_{t-1}/L$, then we get $u_t-u_{t-1}=\frac{1-L}{1-\rho_1 L}\nu_t$.}
        \item Monte-Carlo experiments: They generated 25000 sequences of 720 returns, with each return sequence generated by drawing 720 pairs of standard normal variates $(\sigma^2_{\epsilon},\sigma^2_{\nu})$. They set $\sigma^2_{\epsilon}=1$, thus the variance of returns is $1+\frac{2\sigma^2_{\nu}}{1+\rho_1}$, and the "importance" of the transitory component $p^*_t$ is $\delta = \frac{2\sigma^2_{\nu}}{(1+\rho_1)+2\sigma^2_{\nu}}$. Replication results with comparative statics $\rho_1$ and $\delta$ are shown in Table .
    \end{itemize}
\end{itemize}

\subsubsection{\citet{de1985does}}\label{chap1:sec2:ssec1:paper3}
