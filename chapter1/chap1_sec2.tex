...
\begin{enumerate}
    ...
    $\frac{Cov(l_t,l_{t-1})-Cov(m_t,m_{t-1})}{Var(l_t)} = \frac{Cov(\epsilon_t,\epsilon_{t-1})}{Var(l_t)}+ \frac{Cov(m_t,\epsilon_{t-1})}{Var(l_t)}+Cov(m_{t-1},\epsilon_t)$ can be used to indicate the bounce, 
    where 
