...
\begin{enumerate}
    ...
    Define equally-weighted current midquote index as $eq_t$ and $z_t = cm_t-eq_t$, then decompose the autocovariance of $cm_t$, get:
    $\frac{Cov(cm_t,cm_{t-1})}{Var(l_t)}=\frac{Cov(eq_t,eq_{t-1})}{Var(l_t)}+\frac{Cov(eq_t,z_{t-1})}{Var(l_t)}+\frac{Cov(z_t,eq_{t-1})}{Var(l_t)}+$
\end{enumerate}
