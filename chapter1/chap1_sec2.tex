...
\begin{enumerate}
    ...
    not contaminated by \citet{roll1984simple}'s bid-ask spread. If define the log bid-ask error $l_t-m_t\simeq\Delta(L_t-M_t)$ as $\epsilon_t$. If $\epsilon_t >0$ then 
    $Cov(l_t,l_{t-1})-Cov(m_t,m_{t-1}) = Cov(\epsilon_t,\epsilon_{t-1})+Cov(m_t,\epsilon_{t-1})+Cov(m_{t-1},\epsilon_t)$ can be used to indicate the bounce.
