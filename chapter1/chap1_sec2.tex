...
\citet{froot1995new} first examine a very short-horizon return series: 15-minute returns of S\&P 500 cash index from Feb. 1983 to Dec. 1989. They found 2 main results:
\begin{enumerate}
...
\end{enumerate}
They also consider bid-ask bounce and nontrading effect as the driving force of this decrease in autocorrelation (in other words, separate overreaction from the
information dissemination). 
\begin{enumerate}
    \item[-] \textbf{\textit{Bid-ask bounce}}: They examined the changes in the serial correlations of $l_t=\ln(L_t/L_{t-1})$, the return index of last-trade prices, and $m_t=\ln(M_t/M_{t-1})$,
    the return index of extant midquotes for each last-trade price. Notice that $L_t-M_t$ measures the distance between the last trade index and the center of the then-prevailing spread and $M_t$ is
    not contaminated by \citet{roll1984simple}'s bid-ask spread. 
    
    Define the log bid-ask error $l_t-m_t\simeq\Delta(L_t-M_t)$ as $\epsilon_t$. If $\epsilon_t >0$, there is a greater fraction of buys
    ...
    where: 
    \begin{enumerate}
        \item[-] $\frac{Cov(\epsilon_t,\epsilon_{t-1})}{Var(l_t)}$ is a pure measure of Roll-type bounce. They report that it decline marginally from 1983 to 1988, while the self-autocovariance $\frac{\sum^N_i(\omega^i)^2Cov(l_t^i,l_{t-1}^i)}{Var(l_t)}$
        of the index (squared-weighted sum of self-ARs of all stocks). 
